"""
API Routes for Agent V5: Context-First Agentic Architecture.

Endpoints:
- /query: Main query endpoint with context pre-loading
- /context: Get/clear conversation context
- /health: Health check
- /compare: A/B comparison for research
- /analyze-intent: Debug endpoint for query understanding
"""
...
# Create blueprint
agent_v5_bp = Blueprint('agent_v5', __name__, url_prefix='/api/v5/agent')
...
@agent_v5_bp.route('/health', methods=['GET'])
def health_check():
    """Health check endpoint."""
    return jsonify({
        'status': 'healthy',
        'agent_version': 'v5',
        'design': 'context-first-agentic',
...
@agent_v5_bp.route('/query', methods=['POST'])
def query():
    """
    Main query endpoint with context pre-loading.
    ...
    Response:
    {
        ...
        "model": "gpt-4o",
        "success": true/false,
        "query_intent": {...},  // V5: classified intent
        "context_preloaded": {...}  // V5: what was pre-loaded
    }
    """
    try:
        ...
        logger.info(f"[Agent V5] Query: '{query_text}' (mode={mode}, model={model or 'default'}, conversation={conversation_id[:8]})")
        ...
        response = {
            ...
            # References - what data the agent looked at
            'references': references,

            # V5-specific: Query understanding and context info
            'query_intent': result.get('query_intent'),
            'context_preloaded': result.get('context_preloaded'),
            ...
            'is_direct_response': len(result.get('tools_used', [])) == 0 and result.get('context_preloaded', {}).get('mode') != 'none',
...
@agent_v5_bp.route('/analyze-intent', methods=['POST'])
def analyze_intent():
    """
    Debug endpoint to see how a query is classified.

    Request body:
    {
        "query": "Your question here",
        "conversation_id": "optional" // for context awareness
    }
    ...
    Response:
    {
        "query": "...",
        "intent": {
            "type": "describe|compare|explain|speaker|explore|search",
            "retrieval_mode": "structured|semantic|contrastive|hybrid|agentic_only",
            "entities": {...},
            "metric_focus": "...",
            "target_collections": [...]
        }
    ...
    try:
        ...
        # Analyze intent
        intent = understand_query(query_text, conversation_context)
        ...
        return jsonify({
            ...
            'intent': {
                ...
                'entities': {
                    ...
                    'session_names': intent.session_names,
                    'speaker_names': intent.speaker_names,
                    'topics': intent.topics
                },
                'metric_focus': intent.metric_focus,
                'target_collections': intent.target_collections,
        ...
        logger.error(f"Analyze intent error: {e}", exc_info=True)
        return jsonify({'error': str(e)}), 500


@agent_v5_bp.route('/context', methods=['GET'])
def get_context():
    ...
    return jsonify({
        ...
        'context': {
            ...
            'last_intent': context.get('last_intent')  # V5: also track last query intent
        }
    })


@agent_v5_bp.route('/context', methods=['DELETE'])
def clear_context():
...
@agent_v5_bp.route('/compare', methods=['POST'])
def compare_modes():
    """
    ...
    Response:
    {
        "query": "...",
        "model": "...",
        "baseline": { answer, tools_used, turn_count, context_preloaded },
        "enhanced": { answer, tools_used, turn_count, context_preloaded }
    }
    """
    try:
        data = request.get_json()
        ...
        model = data.get('model')
        logger.info(f"[Agent V5] Compare: '{query_text}' (model={model or 'default'})")
        ...
        return jsonify({
            ...
            'baseline': {
                ...
                'context_preloaded': baseline_result.get('context_preloaded')
            },
            'enhanced': {
                'answer': enhanced_result.get('answer', ''),
                'tools_used': enhanced_result.get('tools_used', []),
                'turn_count': enhanced_result.get('turn_count', 0),
                'query_intent': enhanced_result.get('query_intent'),
                'context_preloaded': enhanced_result.get('context_preloaded')
            }
        })
...
@agent_v5_bp.route('/tools', methods=['GET'])
def list_tools():
    """List available tools for each mode."""
    from agent_v4.tools import TOOL_SCHEMAS, BASELINE_TOOL_SCHEMAS
...
@agent_v5_bp.route('/conversations', methods=['GET'])
...
@agent_v5_bp.route('/conversations/<conversation_id>', methods=['GET'])
...
@agent_v5_bp.route('/conversations/<conversation_id>', methods=['DELETE'])
...
@agent_v5_bp.route('/conversations', methods=['POST'])
