"""
API Routes for BLINC Agent V7

Full Context Agent - All truncation limits removed.
Based on V3 with complete data access for LLM.
"""
...
def query():
    ...
    try:
        ...
        logger.info(f"[Agent V7] Query: '{query_text}' (conversation={conversation_id})")
...
@agent_v7_bp.route('/metrics', methods=['GET'])
def get_metrics():
    """
...
@agent_v7_bp.route('/cache', methods=['DELETE'])
def clear_cache():
...
def list_conversations():
    ...
    try:
        ...
        conversations = db_helper.get_agent_conversations(user_id=user_id, agent_version='v7')
