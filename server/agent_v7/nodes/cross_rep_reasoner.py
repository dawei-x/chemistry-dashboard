...
CROSS_REP_SYSTEM_PROMPT = """You are an expert analyst reasoning across your own prior analytical work.

## Your Prior Analysis Work

You have previously analyzed this collaborative discussion and created several artifacts:

**Transcript Analysis**: You reviewed the full discussion, noting what each participant said,
their communication patterns, and linguistic indicators of thinking depth.

**Concept Map**: You constructed a visual representation of how ideas connect, identifying
key concepts, their relationships (causal, supporting, contrasting), and reasoning chains.

**7C Collaboration Analysis**: You evaluated the group's collaboration quality across seven
dimensions, coding specific transcript segments as evidence for each score.

Now you need to reason across these artifacts you created to find where they converge,
where they show tension, and what story they tell together.

## Your Interpretive Role

Don't just mechanically list convergence/tension - INTERPRET what YOUR ANALYSIS means:
- What patterns emerge when you look across your different analytical lenses?
- What does a high 7C score combined with sparse concept map connections suggest?
- When transcript shows hesitation but your concept map shows creative leaps, what's happening?

## Epistemic Hierarchy (weight your evidence accordingly)

1. TRANSCRIPT (primary) - What was actually said - your ground truth source
2. CONCEPT_MAP (derived) - Structure of ideas you extracted - shows reasoning patterns
3. COLLABORATION (interpreted) - 7C scores you assigned - quantifies interaction quality
4. SPEAKER_PROFILE (aggregated) - Patterns you observed across sessions
5. SESSION_OVERVIEW (summary) - High-level context you noted
    ...
    for sg in sub_goals:
        ...
        for i, step in enumerate(steps):
            ...
            if tool_result.get('results'):
                # Legacy search results format - NO TRUNCATION
                ...
                # THEN: Include ALL individual nodes - NO TRUNCATION
                ...
                # NO TRUNCATION - include full explanations and ALL coded segments
                ...
                if isinstance(dims, dict):
                    for dim_name, dim_data in dims.items():
                        ...
                        # Add score + FULL explanation (no truncation)
                        results_to_show.append({
                            'text': f"[7C {dim_name}] Score: {score}/100 - {explanation}",
                            '_session_id': session_id,
                            '_session_name': session_name
                        })

                        # Add ALL coded segments (no limit) - this is critical evidence
                        for seg in coded_segments:
                            if isinstance(seg, str) and seg.strip():
                                results_to_show.append({
                                    'text': f"[7C {dim_name} evidence]: {seg}",
                                    '_session_id': session_id,
                                    '_session_name': session_name
                                })

            elif tool_result.get('sessions'):
                # search_for_sessions tool format - NO TRUNCATION
                for s in tool_result['sessions']:
                ...
                # compare_sessions tool format - extract comparison data - NO TRUNCATION
                for comparison in tool_result['results']:
                    if isinstance(comparison, dict):
                        ...
                        # Format ALL compared sessions
                        for sess in comparison.get('session_details', []):
                            ...
                            themes = ', '.join(sess.get('main_themes', [])) if sess.get('main_themes') else ''
                ...
                # Legacy get_artifacts format (deprecated but supported) - NO TRUNCATION
            ...
            for j, r in enumerate(results_to_show):
                if isinstance(r, dict):
                    # Safely extract text - NO TRUNCATION for full context
...
def _fast_heuristic_analysis(
    ...
    if len(reps_list) >= 1:
        ...
        convergence_points.append(ConvergencePoint(
            ...
            evidence=evidence,  # NO TRUNCATION - include all evidence
