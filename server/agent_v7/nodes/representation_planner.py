...
def _lookup_speaker_sessions(speaker_name: str) -> List[int]:
    """
    Look up which session(s) a speaker participated in.

    This is critical for speaker-focused queries without session context.
    Without this, semantic search might return content from wrong sessions.

    Args:
        speaker_name: Name of the speaker to look up

    Returns:
        List of session IDs the speaker participated in
    """
    try:
        ...
        cursor.execute("""
            SELECT DISTINCT t.session_device_id
            FROM transcript t
            JOIN speaker s ON t.speaker_id = s.id
            WHERE s.alias LIKE %s
            ORDER BY t.session_device_id
        """, (f"%{speaker_name}%",))
        ...
        session_ids = [r['session_device_id'] for r in results]
        if session_ids:
            logger.info(f"[Speaker Lookup] Found {speaker_name} in sessions: {session_ids}")
        else:
            logger.warning(f"[Speaker Lookup] Speaker '{speaker_name}' not found in any session")

        return session_ids

    except Exception as e:
        logger.error(f"[Speaker Lookup] Database error: {e}")
        return []
...
INDICATOR_TO_TOOL = {
    ...
    # STRUCTURAL SIGNALS - Signal-based discovery (no session required)
    "challenge edges": {"rep": "discovery", "tool": "find_sessions_by_structure", "edge_types": ["challenges"]},
    "contrast edges": {"rep": "discovery", "tool": "find_sessions_by_structure", "edge_types": ["contrasts_with"]},
    "disagreement": {"rep": "discovery", "tool": "find_sessions_by_structure", "edge_types": ["challenges", "contrasts_with"]},
    "debate": {"rep": "discovery", "tool": "find_sessions_by_structure", "edge_types": ["challenges", "contrasts_with"]},
    "counter-arguments": {"rep": "discovery", "tool": "find_sessions_by_structure", "edge_types": ["challenges"]},
    "builds_on edges": {"rep": "discovery", "tool": "find_sessions_by_structure", "edge_types": ["builds_on", "elaborates"]},
    "hypothesis nodes": {"rep": "discovery", "tool": "find_sessions_by_structure", "node_types": ["hypothesis"]},
    "reasoning nodes": {"rep": "discovery", "tool": "find_sessions_by_structure", "node_types": ["reasoning"]},
    "question nodes": {"rep": "discovery", "tool": "find_sessions_by_structure", "node_types": ["question"]},
    "conflict_score": {"rep": "discovery", "tool": "find_sessions_by_structure", "seven_c_dimension": "conflict_score"},
    "contribution_score": {"rep": "discovery", "tool": "find_sessions_by_structure", "seven_c_dimension": "contribution_score"},
}
...
def plan_retrieval(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # V7 FIX: Also detect "all sessions" patterns that need compare_sessions
    all_sessions_pattern = r'\b(all sessions|every session|each session|across all|across sessions)\b'
    has_all_sessions = re.search(all_sessions_pattern, original_query)
    ...
    logger.info(f"  - has_all_sessions: {bool(has_all_sessions)}")

    # V7 FIX: Trigger global comparison for EITHER superlatives OR "all sessions" patterns
    needs_global_comparison = (
        is_comparison and not compared_sessions and has_superlative and has_target
    ) or (
        has_all_sessions and not compared_sessions  # "all sessions" queries
    )
...
def _plan_for_subgoal(
    ...
    # CRITICAL FIX: When we have a speaker but no session, look up the speaker's sessions
    # This prevents semantic search from returning content from wrong sessions
    # Example: "Did David demonstrate systems thinking?" should analyze Session 20 (where David is),
    # not Session 22 (which might have similar content but a different speaker)
    #
    # V7 FIX: For queries about speaker behavior across sessions (e.g., "How does Lex engage?"),
    # we should retrieve from ALL sessions, not just the first one.
    all_speaker_sessions = []
    if speaker_focus and not session_focus:
        speaker_sessions = _lookup_speaker_sessions(speaker_focus)
        if speaker_sessions:
            if len(speaker_sessions) > 1:
                # Speaker appears in multiple sessions - retrieve from ALL of them
                all_speaker_sessions = speaker_sessions
                logger.info(f"  [Speaker→Sessions] {speaker_focus} appears in {len(speaker_sessions)} sessions: {speaker_sessions}")
                # Use first session as primary focus, but we'll add steps for others
                session_focus = speaker_sessions[0]
            else:
                session_focus = speaker_sessions[0]
                logger.info(f"  [Speaker→Session] Resolved {speaker_focus} to session {session_focus}")
    ...
    # V7 FIX: Add retrieval steps for additional sessions when speaker appears in multiple
    # This ensures queries like "How does Lex engage?" get data from ALL sessions
    if all_speaker_sessions and len(all_speaker_sessions) > 1:
        # We already handled the first session above, now add steps for the rest
        for additional_session in all_speaker_sessions[1:]:
            logger.info(f"  Adding transcript retrieval for additional session {additional_session}")
            steps.append(RetrievalStep(
                representation='transcript',
                purpose=f'Additional session where {speaker_focus} participated',
                tool='get_transcript',
                parameters={'session_id': additional_session},
                priority='secondary'
            ))
            # Also get 7C for comparison if we have '7c' or 'collaboration' in representations
            if primary_rep in ['7c', 'collaboration'] or '7c' in secondary_reps:
                steps.append(RetrievalStep(
                    representation='7c',
                    purpose=f'Collaboration analysis for session {additional_session}',
...
def _create_step_for_representation(
    ...
    # === NEW: Check if indicators suggest structural signal-based discovery ===
    if not session_focus and indicators:
        indicators_lower = ' '.join(str(i).lower() for i in indicators)
        description_lower = description.lower()
        combined = indicators_lower + ' ' + description_lower

        # Check for structural signal patterns
        for pattern, mapping in INDICATOR_TO_TOOL.items():
            if pattern in combined and mapping.get('tool') == 'find_sessions_by_structure':
                tool_name = 'find_sessions_by_structure'
                tool_info = mapping
                logger.info(f"Matched structural signal '{pattern}' -> find_sessions_by_structure")
                break
    ...
    elif tool_name == 'find_sessions_by_structure':
        # Signal-based session discovery (edge types, node types, 7C dimensions)
        params = {'top_k': 5}
        # Copy structural parameters from tool_info if available
        if tool_info.get('edge_types'):
            params['edge_types'] = tool_info['edge_types']
        if tool_info.get('node_types'):
            params['node_types'] = tool_info['node_types']
        if tool_info.get('seven_c_dimension'):
            params['seven_c_dimension'] = tool_info['seven_c_dimension']
            params['min_seven_c_score'] = tool_info.get('min_score', 50)
        logger.info(f"Signal-based discovery with params: {params}")
