...
## Signal-Aware Session Discovery

For PATTERN queries (e.g., "What leads to productive disagreement?", "Which sessions show deep reasoning?"), you need to identify the right DATA SIGNALS that indicate those patterns.

**Concept Map Edge Types → Behavioral Signals:**
- `challenge` edges: Direct disagreement, counter-arguments, questioning claims
- `builds_on` edges: Elaboration, agreement with extension, constructive development
- `contrast` edges: Comparing different views, noting distinctions
- `support` edges: Evidence provision, agreement, backing claims
- `caused_by` / `leads_to` edges: Causal reasoning, consequence exploration

**Concept Map Node Types → Thinking Signals:**
- `question` nodes: Inquiry-driven discourse
- `hypothesis` nodes: Speculative thinking, tentative claims
- `reasoning` nodes: Explicit logical reasoning
- `synthesis` nodes: Integrating multiple ideas

**7C Dimensions → Collaboration Signals:**
- `conflict_score`: Presence of productive disagreement (high = more disagreement managed)
- `contribution_score`: Participation balance
- `climate_score`: Psychological safety
- `constructive_score`: Building on ideas
- `communication_score`: Clarity of expression

**Pattern → Signal Mapping (use this to guide session discovery):**
| Pattern Query | Primary Signals | Secondary Signals |
|--------------|-----------------|-------------------|
| "productive disagreement" | challenge edges, contrast edges | conflict_score, question nodes |
| "deep reasoning" | builds_on edges, reasoning nodes | hypothesis nodes, causal edges |
| "balanced participation" | contribution_score | speaker utterance counts |
| "idea development" | builds_on edges, synthesis nodes | constructive_score |
| "critical thinking" | challenge edges, question nodes | reasoning nodes |

**For pattern discovery queries:**
1. Include a sub-goal to search CONCEPT_MAP for sessions with relevant edge/node types
2. Include a sub-goal to filter sessions by relevant 7C scores
3. Then get transcript evidence from top candidate sessions
...
**Query: "What patterns lead to productive disagreement?"**
This is a PATTERN DISCOVERY query - use signal-awareness to find the right sessions:
```json
{
    "is_simple_query": false,
    "abstract_constructs": ["productive disagreement"],
    "operationalization": {
        "productive disagreement": [
            "challenge edges in concept map",
            "contrast edges showing different views",
            "high conflict_score in 7C",
            "question nodes prompting reconsideration"
        ]
    },
    "sub_goals": [
        {
            "id": "sg1",
            "description": "Find sessions with challenge and contrast edges indicating disagreement",
            "indicators": ["challenge edges", "contrast edges", "questioning", "counter-arguments"],
            "primary_representation": "concept_map",
            "secondary_representations": [],
            "session_filter": null,
            "speaker_filter": null
        ...
        {
            "id": "sg2",
            "description": "Get 7C conflict scores to identify sessions with managed disagreement",
            "indicators": ["conflict_score", "climate_score", "constructive_score"],
            "primary_representation": "collaboration",
            "secondary_representations": [],
            "session_filter": null,
            "speaker_filter": null
        ...
        {
            "id": "sg3",
            "description": "Get transcript evidence showing how disagreements were expressed and resolved",
            "indicators": ["disagreement language", "resolution patterns", "perspective shifts"],
            "primary_representation": "transcript",
            "secondary_representations": [],
            "session_filter": null,
            "speaker_filter": null
...
def _ensure_collaboration_subgoal(
    ...
    collaboration_indicators = [
        ...
        # V7 FIX: Include 7C dimension names to trigger 7C analysis
        r'\bclimate\b',  # 7C dimension: psychological safety
        r'\bcommunication\b',  # 7C dimension: clarity, listening
        r'\bconflict\b',  # 7C dimension: disagreement handling
        r'\bcontext\b.*(?:aware|understand)',  # 7C dimension: shared understanding
        r'\bconstructive\b',  # 7C dimension: building on ideas
        r'\bcompatibility\b',  # 7C dimension: work style alignment
        # Also trigger on strengths/weaknesses pattern (implies 7C evaluation)
        r'\b(?:strength|weakness)s?\b.*\b(?:across|session|all)\b',
    ]
...
def _determine_fast_path_tool(query: str, state: Dict[str, Any]) -> tuple:
    ...
    # V7 FIX: FIRST check for collaboration/7C keywords - these need PRAS reasoning, not fast path
    # This must come BEFORE list_sessions patterns to avoid mis-routing
    collab_exclusion_keywords = [
        'collaboration', 'collaborate', 'collaborative',
        'engagement', 'engaged', 'engaging',
        'participation', 'interact', 'interaction',
        '7c', 'seven c',
        # All 7C dimension names
        'contribution', 'climate', 'conflict',
        'constructive', 'context', 'compatibility',
        'communication',
        # Evaluation terms that imply 7C analysis
        'strength', 'weakness', 'strengths', 'weaknesses',
        'productive', 'quality'
    ]
    for keyword in collab_exclusion_keywords:
        if keyword in query_lower:
            logger.info(f"[Fast Path] Excluded: collaboration keyword '{keyword}' requires PRAS + 7C analysis")
            return (None, None)  # Route to PRAS for proper 7C analysis
    ...
    collaboration_keywords = [
        ...
        # V7 FIX: All 7C dimension names should trigger PRAS + 7C analysis
        'contribution', 'climate', 'conflict',
        'constructive', 'context', 'compatibility',
        'communication',  # 7C dimension
        # Strengths/weaknesses imply 7C evaluation
        'strength', 'weakness', 'strengths', 'weaknesses'
    ]
