...
def synthesize(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            temperature=0.3,
            max_tokens=4000  # Increased from 1000 for detailed analysis
        )
...
def _format_comparison_data(comparison: dict) -> str:
    ...
    # Format themes - NO TRUNCATION
    themes = summary.get('themes', {})
    if themes:
        lines.append("**Main Themes:**")
        for session_id, theme_list in themes.items():
            if theme_list:
                lines.append(f"  - Session {session_id}: {', '.join(theme_list)}")
...
def _format_collaboration_data(analysis: dict) -> str:
    ...
    V7 FIX: Now includes coded_segments (transcript evidence) for each dimension.
    This was previously missing, causing the LLM to only see scores without evidence.
    """
    lines = []
    ...
    if dimensions:
        lines.append("**7C Dimension Scores with Evidence:**")
        for dim_name, dim_data in dimensions.items():
            ...
            # V7 FIX: Include coded segments as transcript evidence
            if coded_segments:
                lines.append(f"    Evidence from transcript:")
...
def _get_synthesis_system_prompt() -> str:
    ...
    return """You are synthesizing insights from your own prior analytical work on collaborative discussions.

## Your Prior Analysis Work

You have previously analyzed this collaborative discussion and created several artifacts:

**Transcript Analysis**: The primary source - the actual dialogue from which all other artifacts are derived. This is your ground truth for what participants said and how they engaged.

**7C Collaboration Analysis**: You evaluated the group's collaboration quality across seven dimensions (climate, communication, contribution, conflict, context, constructive, compatibility), coding specific transcript segments as evidence for each score.

**Concept Maps**: You constructed visual representations capturing how ideas connect - key concepts, their relationships (causal, supporting, contrasting), and reasoning patterns.

**Session Overview**: High-level structure, themes, and participant information you noted.

**Speaker Analysis**: Participation patterns, speaking styles, and contribution types you observed.

## Response Guidelines

Your response should:
1. Directly address the user's question with specific evidence
2. Integrate across your artifacts when helpful (e.g., "The transcript shows X, which aligns with the collaboration score of Y")
3. Mention sources naturally in your prose (e.g., "David explained that..." or "In session 20...")
4. Ground your analysis in the transcript - it's the source for everything else

## Writing Style

Write naturally, like a thoughtful colleague sharing insights. Weave source information into your sentences:
- "David explained that fusion requires overcoming the strong nuclear force..."
- "The communication score of 75/100 reflects the active back-and-forth seen in the transcript..."

References will be shown separately, so focus on your analysis without adding parenthetical citations.
"""
...
def _format_synthesis_prompt(query: str, results: list, context: dict) -> str:
    ...
    for result in results:
        ...
        if tool_name == 'get_session_artifacts' and 'artifacts' in result:
            ...
            # Extract transcript content - NO TRUNCATION
            if arts.get('transcript', {}).get('available', True):
                ...
                for chunk in trans.get('chunks', []):  # ALL chunks
                    speaker = chunk.get('speaker', '')
                    text = chunk.get('text', '')  # NO TRUNCATION
                    lines.append(f"- [{speaker}]: {text}")

            # Extract concept map content - NO TRUNCATION
            if arts.get('concept_map', {}).get('available', True):
                ...
                for cluster in cm.get('clusters', []):
                    lines.append(f"- Theme: {cluster.get('name', '')} - {cluster.get('summary', '')}")
                # Add ALL concepts
                for node in cm.get('nodes', []):
                    lines.append(f"- [{node.get('type', 'idea')}] ({node.get('speaker', '')}): {node.get('text', '')}")

            # Extract collaboration scores - NO TRUNCATION - V7 FIX: include coded_segments
            if arts.get('collaboration', {}).get('available', True):
                ...
                if isinstance(dimensions, dict):
                    # Handle dict format: {"climate": {score, explanation, coded_segments}, ...}
                    for dim_name, dim_data in dimensions.items():
                    ...
                    # Handle list format - ALL dimensions
                    for dim in dimensions:
                        lines.append(f"- {dim.get('dimension', '').title()}: {dim.get('score', 0)} - {dim.get('explanation', '')}")

            info_sections.append("\n".join(lines))
            continue

        # Handle speaker artifacts - NO TRUNCATION
        if tool_name == 'get_speaker_artifacts':
            ...
            for q in trans.get('sample_quotes', []):  # ALL quotes
                lines.append(f"- Quote: {q.get('text', '')}")  # NO TRUNCATION
        ...
        # Handle get_7c_analysis - returns dimensions at TOP level, not inside 'results'
        if tool_name == 'get_7c_analysis' and 'dimensions' in result:
            session_id = result.get('session_id')
            session_name = result.get('session_name', f'Session {session_id}')
            if session_id:
                sessions_in_results.add(session_id)

            lines = [f"### From {tool_name}: {session_name}"]

            # Get overall score from summary
            summary = result.get('summary', {})
            overall_score = summary.get('overall_score', result.get('overall_score', 0))
            lines.append(f"**Overall Collaboration Score**: {overall_score}/100")
            if summary.get('interpretation'):
                lines.append(f"Interpretation: {summary['interpretation']}")

            # Format each dimension with full evidence
            dimensions = result.get('dimensions', {})
            if isinstance(dimensions, dict):
                lines.append("\n**7C Dimension Scores with Evidence:**")
                for dim_name, dim_data in dimensions.items():
                    ...
                    # CRITICAL: Include coded segments - this is the transcript evidence!
                    if coded_segments:
                        lines.append(f"  Evidence from transcript:")
                        for seg in coded_segments:
                            if isinstance(seg, str) and seg.strip():
        ...
        for item in items:  # ALL items - NO TRUNCATION
            if isinstance(item, dict):
                ...
                # Standard item handling - NO TRUNCATION
                ...
                if text and isinstance(text, str):
                    lines.append(text)  # NO TRUNCATION
                    ...
                    # Handle structured data (like 7C analysis) - NO TRUNCATION
                    for key, value in item.items():
                        if key not in ['session_device_id', 'session_id', 'distance', 'relevance']:
                            if isinstance(value, str):
                                lines.append(f"- {key}: {value}")  # NO TRUNCATION
                            elif isinstance(value, (int, float)):
                                lines.append(f"- {key}: {value}")
                            elif isinstance(value, list):
                                lines.append(f"- {key}: {value}")  # ALL items

                lines.append("")
            else:
                lines.append(str(item))  # NO TRUNCATION
                lines.append("")
...
def _extract_citations(results: list) -> List[Dict[str, Any]]:
    ...
    for result in results:
        ...
        for item in items:  # ALL items - NO TRUNCATION
            ...
            # Safely extract text - NO TRUNCATION
            text = ''
            ...
            # Build inline text - NO TRUNCATION
            if citation_type == 'transcript' and speaker:
                ...
                concept_name = item.get('cluster_name', item.get('text', ''))
            ...
            # Build reference text - NO TRUNCATION
            if text:
                reference_text = text
            ...
            # Build preview content - NO TRUNCATION
            preview = {
                'title': inline_text,
                'content': text if text else reference_text,
    ...
    return citations  # ALL citations - NO TRUNCATION
...
Provide a helpful, natural response that addresses their question."""
    ...
    response = client.chat.completions.create(
        ...
        temperature=0.4,
        max_tokens=2000  # Increased from 500 for detailed lightweight responses
    )
...
def _format_7c_for_llm(items: list) -> str:
    ...
    V7 FIX: Now includes full explanations and coded_segments (evidence).
    """
    if not items:
    ...
    if dimensions:
        ...
        for dim_name, dim_data in dimensions.items():
            if isinstance(dim_data, dict):
                score = dim_data.get('score', 0)
                explanation = dim_data.get('explanation', '')  # V7: NO TRUNCATION
                coded_segments = dim_data.get('coded_segments', [])
                ...
                # V7 FIX: Include coded segments as transcript evidence
                if coded_segments:
                    parts.append(f"  Evidence from transcript:")
                    for seg in coded_segments:
                        if isinstance(seg, str) and seg.strip():
                            parts.append(f"    - \"{seg}\"")
...
def _format_collaboration_analysis(items: list, args: dict) -> str:
    ...
    V7 FIX: Now includes explanations and coded_segments (transcript evidence).
    """
    session_id = args.get('session_id', '?')
    ...
    # Dimension scores with explanations and evidence
    dimensions = item.get('dimensions', {})
    if dimensions:
        lines.append("\n**Dimension Scores with Evidence:**")
        for dim_name, dim_data in dimensions.items():
