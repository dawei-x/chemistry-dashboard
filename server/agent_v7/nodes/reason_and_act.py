...
For meaningful comparison analysis:
1. Get transcripts for each session using `get_transcript(session_id=X)` - this is your primary evidence for behavioral analysis
2. Use `compare_sessions(session_ids={sessions})` to get metrics and 7C scores as context
3. Analyze the dialogue directly - what behaviors do you observe? How do participants interact?

The session IDs {sessions} are already resolved. Use them directly.

Important: Don't search for session names as content (e.g., don't search for "nuclear fusion" - that's a session name, not transcript content).
"""
