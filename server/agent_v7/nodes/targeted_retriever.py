...
from ..tools.artifact_tools import search_for_sessions
...
def _extract_search_keywords(query: str) -> str:
    ...
    Removes common question words to get the semantic core of the query.
    """
    # Common question/filler words to remove
    stop_words = {
        'what', 'was', 'is', 'are', 'were', 'said', 'about', 'how', 'why', 'when',
        'where', 'who', 'which', 'the', 'a', 'an', 'and', 'or', 'but', 'in', 'on',
        'at', 'to', 'for', 'of', 'with', 'by', 'from', 'do', 'does', 'did', 'can',
        'could', 'would', 'should', 'be', 'been', 'being', 'have', 'has', 'had',
        'tell', 'me', 'us', 'show', 'find', 'give', 'get', 'let', 'please', 'help'
    }

    words = query.replace('?', '').replace('!', '').replace('.', '').split()
    # Keep words that are not stop words, with special handling for acronyms (uppercase 2-letter words)
    keywords = []
    for w in words:
        w_lower = w.lower()
        if w_lower not in stop_words:
            # Keep if word is longer than 2 chars, OR if it's all uppercase (acronym like "AI")
            if len(w) > 2 or w.isupper():
                keywords.append(w)

    if keywords:
        return ' '.join(keywords)
    return query  # Fallback to original if no keywords extracted


def _filter_sessions_by_original_query(
    session_ids: List[int],
    original_query: str,
    min_relative_score: float = 0.70
) -> List[int]:
    """
    Filter discovered sessions by relevance to the ORIGINAL user query.

    This handles the case where multiple search queries (e.g., one for transcripts,
    one for concepts) discover different sessions, but not all are relevant to
    the original user question.

    Args:
        session_ids: List of session IDs discovered by various search steps
        original_query: The original user query
        min_relative_score: Sessions must score >= this fraction of the best match

    Returns:
        Filtered list of session IDs that are relevant to the original query
    """
    if not session_ids:
        return []

    # V7 FIX: "All sessions" queries should NOT be filtered - return all discovered sessions
    ...
    for pattern in all_sessions_patterns:
        if pattern in query_lower:
            logger.info(f"[Global Filter] BYPASSED: Query contains '{pattern}' - returning all {len(session_ids)} discovered sessions")
            return session_ids

    # Extract keywords for more accurate semantic matching
    search_query = _extract_search_keywords(original_query)
    logger.info(f"[Global Filter] Validating {len(session_ids)} sessions against keywords: '{search_query}' (from: '{original_query}')")

    # Get relevance scores using extracted keywords
    search_result = search_for_sessions(
        query=search_query,
        top_k=10,  # Get enough to cover all discovered sessions
        min_score=0.10,  # Low absolute threshold
        min_relative_score=min_relative_score  # Apply relative filtering
    )

    # Extract session IDs that passed the relevance filter
    relevant_ids = set()
    for s in search_result.get('sessions', []):
        sid = s.get('session_id')
        if sid:
            relevant_ids.add(int(sid))
    logger.info(f"  [Global Filter] Search returned relevant session IDs: {relevant_ids}")

    # Filter discovered sessions to only include relevant ones
    filtered = []
    for sid in session_ids:
        if sid in relevant_ids:
            filtered.append(sid)
        else:
            logger.info(f"  [Global Filter] Excluded session {sid} - not relevant to original query")

    logger.info(f"  [Global Filter] Kept {len(filtered)}/{len(session_ids)} sessions: {filtered}")
    return filtered
...
def targeted_retrieve(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # If this was a session discovery call, extract discovered sessions
    # and queue them for artifact fetching
    discovered_sessions = state.get('_discovered_sessions', [])
    if tool_name in ('search_for_sessions', 'find_sessions_by_structure'):
        ...
        for s in sessions:
            ...
            if sid and sid not in discovered_sessions:
                ...
                signal_info = ""
                if s.get('edge_counts'):
                    signal_info = f" (edges: {s.get('edge_counts')})"
                elif s.get('signal_strength'):
                    signal_info = f" (signal: {s.get('signal_strength')})"
                logger.info(f"  [Discovery] Found session {sid}: {s.get('session_name', 'Unknown')}{signal_info}")
...
def _normalize_session_id(value):
    ...
    if isinstance(value, str):
        ...
        # Second pass: substring match (normalized contains name OR name contains normalized)
        for name, sid in session_mapping:
...
def _fetch_discovered_artifacts(state: Dict[str, Any], session_ids: List[int]) -> Dict[str, Any]:
    ...
    # GLOBAL RELEVANCE FILTER: Validate all discovered sessions against the ORIGINAL query
    # This handles the case where multiple search queries (e.g., one for transcripts,
    # one for concepts) discover sessions that aren't actually relevant to the user's question.
    original_query = state.get('original_query', state.get('current_query', ''))
    logger.info(f"[Global Filter] Original query from state: '{original_query}'")
    if original_query:
        sessions_to_fetch = _filter_sessions_by_original_query(
            session_ids,
            original_query,
            min_relative_score=0.70
        )
    else:
        sessions_to_fetch = session_ids
    ...
    for sid in sessions_to_fetch:
        ...
        for sg in sub_goals:
            ...
            if sg_id in subgoal_results:
                ...
                # Add concept map step record (CRITICAL for synthesis to have nodes/edges)
                if concept_result:
                    concept_step = {
                        'step': {
                        ...
                        'tool_result': concept_result,
                        'grade': {'is_relevant': True, 'relevance_score': 0.85},
                        'reflection': {
                            'subgoal_satisfied': True,
                            'satisfaction_level': 'full',
                            'evidence_summary': f'Concept map from session {sid}: {concept_result.get("summary", {}).get("total_nodes", 0)} nodes, {concept_result.get("summary", {}).get("total_edges", 0)} edges'
                        }
                    }
                    subgoal_results[sg_id]['steps_executed'].append(concept_step)
