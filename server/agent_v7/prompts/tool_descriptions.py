...
Prefer this over get_artifacts when you only need concept map data.
...
- dimensions[dim_name].coded_segments: Specific evidence from the discussion
  ...
  → Include these when discussing scores to ground your analysis
...
3. Include context: "Conflict score: 60/100, reflecting [cite specific behaviors from coded_segments]"
""",
        "parameters": {
...
- Does BFS graph traversal automatically
...
When not to clarify (prefer searching instead):
...
When in doubt, search rather than ask. Users generally prefer results over questions.
""",
        "parameters": {
...
""",
    ...
    "compare_sessions": {
        "description": """
Compare multiple sessions across multiple dimensions.
...
For single sessions, prefer get_session_overview. For finding specific content, prefer search_transcripts.
...
- Key differences, similarities, and rankings
""",
        "parameters": {
...
Report both supporting and countering evidence to give a balanced assessment.
""",
        "parameters": {
